...
async def test_llm_classification():
    ...
    for query in test_queries:
        ...
        print(f"  Distribution: {', '.join([f'{p.value}: {s:.1%}' for p, s in result_rules.distribution.items()])}")
        ...
        try:
            ...
            print(f"  Distribution: {', '.join([f'{p.value}: {s:.1%}' for p, s in result_hybrid.distribution.items()])}")
            
            # Show LLM reasoning if available
            if result_hybrid.reasoning.get(result_hybrid.primary_paradigm):
                llm_reasoning = [r for r in result_hybrid.reasoning[result_hybrid.primary_paradigm] if r.startswith("LLM:")]
                if llm_reasoning:
