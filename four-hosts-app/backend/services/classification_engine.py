...
class QueryAnalyzer:
    ...
    def _identify_domain(self, query: str, entities: List[str]) -> Optional[str]:
        ...
        query_lower = query.lower()
        domain_keywords = {
            'technology': ['technology', 'software', ' ai ', 'digital', 'cyber', 'machine learning'],
            'business': ['business', 'company', 'market', 'profit', 'revenue', 'strategy'],
            'healthcare': ['health', 'medical', 'patient', 'doctor', 'disease', 'care'],
            'education': ['education', 'school', 'student', 'learning', 'teach'],
            'social_justice': ['justice', 'rights', 'equality', 'discrimination', 'injustice'],
    ...
    def _score_urgency(self, query: str, tokens: List[str]) -> float:
        ...
        urgency_indicators = ['urgent', 'urgently', 'immediately', 'now', 'asap', 'quickly', 'emergency', 'critical', 'crisis', 'desperate']
    ...
    def _score_emotional_valence(self, tokens: List[str]) -> float:
        ...
        positive_words = ['help', 'support', 'love', 'care', 'hope', 'success', 'happy', 'grateful', 'inspire', 'encourage']
        negative_words = ['fight', 'attack', 'hate', 'angry', 'unfair', 'wrong', 'corrupt', 'evil', 'destroy', 'victim']
...
class ParadigmClassifier:
    ...
    async def classify(self, query: str) -> ClassificationResult:
        ...
        secondary = sorted_paradigms[1][0] if len(sorted_paradigms) > 1 and sorted_paradigms[1][1] > 0.2 else None
    ...
    def _rule_based_classification(self, query: str, features: QueryFeatures) -> Dict[HostParadigm, ParadigmScore]:
    ...
    def _score_intent_alignment(self, paradigm: HostParadigm, intent_signals: List[str]) -> float:
    ...
    def _combine_scores(self, rule_scores: Dict[HostParadigm, ParadigmScore],
        ...
        combined = {}
        for paradigm in HostParadigm:
            rule_score = rule_scores.get(paradigm, ParadigmScore(paradigm=paradigm, score=0, confidence=0, reasoning=[], keyword_matches=[]))
            ...
            combined[paradigm] = ParadigmScore(
                paradigm=paradigm,
                score=total_score,
                confidence=(rule_score.confidence + llm_scores.get(paradigm, rule_score).confidence) / 2 if llm_scores else rule_score.confidence,
