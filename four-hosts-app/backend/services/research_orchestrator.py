...
class CostMonitor:
    ...
    async def check_budget_alerts(self, daily_budget: float = 50.0) -> List[str]:
        ...
        total_cost = sum(
            api_data.get('cost', 0.0) 
            for api_data in costs.values()
        )
        
        if total_cost > daily_budget * 0.8:
            alerts.append(f"Daily budget at {total_cost/daily_budget*100:.1f}% (${total_cost:.2f})")
        
        if total_cost > daily_budget:
            alerts.append(f"Daily budget exceeded: ${total_cost:.2f} > ${daily_budget:.2f}")
...
class ParadigmAwareSearchOrchestrator:
    ...
    async def execute_paradigm_research(self, 
                                      ...
                                      max_results: int = 100) -> ResearchExecutionResult:
        """
        ...
        for query_data in search_queries:
            ...
            cached_results = await get_cached_search_results(query, config_dict, paradigm)
            ...
            if cached_results:
                ...
                try:
                    ...
                    api_results = await self.search_manager.search_with_fallback(query, config)
                    ...
                    for result in api_results:
                        result.credibility_score = result.credibility_score * weight if hasattr(result, 'credibility_score') else weight
        ...
        for result in filtered_results[:20]:  # Top 20 results
            try:
                ...
                logger.warning(f"Credibility check failed for {result.domain}: {str(e)}")
    ...
    async def get_execution_stats(self) -> Dict[str, Any]:
        ...
        for result in self.execution_history:
            paradigm_counts[result.paradigm] = paradigm_counts.get(result.paradigm, 0) + 1
...
async def execute_research(context_engineered_query, max_results: int = 50) -> ResearchExecutionResult:
    """Convenience function to execute research"""
    return await research_orchestrator.execute_paradigm_research(context_engineered_query, max_results)
...
async def test_research_orchestration():
    ...
    mock_select_output.search_queries = [
        {"query": "small business compete Amazon strategy", "type": "strategic", "weight": 1.0},
        {"query": "Amazon competitive disadvantages", "type": "strategic", "weight": 0.8},
        {"query": "local business advantages over Amazon", "type": "strategic", "weight": 0.9}
    ]
    ...
    try:
        ...
        print(f"  Processing time: {result.execution_metrics['processing_time_seconds']:.2f}s")
        ...
        for i, res in enumerate(result.filtered_results[:3], 1):
            ...
            print(f"     Domain: {res.domain} (credibility: {res.credibility_score:.2f})")
            print(f"     URL: {res.url}")
