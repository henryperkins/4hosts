...
class RateLimiter:
    ...
    async def check_rate_limit(
        ...
        if not await self._check_concurrent_limit(identifier, limits["concurrent_requests"]):
            return False, {
    ...
    def get_usage_stats(self, identifier: str, role: UserRole) -> Dict[str, Any]:
        ...
        for window_name, window_seconds, limit_key in [
            ...
            if self.redis_client:
                ...
                window_start = current_time - window_seconds
                count = sum(1 for ts in self.request_history[identifier] if ts > window_start)
...
class RateLimitMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            response.headers["X-RateLimit-Limit"] = str(stats["limits"]["requests_per_minute"])
...
class AdaptiveRateLimiter:
    ...
    async def check_rate_limit(
        ...
        try:
            return await self.base_limiter.check_rate_limit(identifier, role, request_type)
        finally:
...
class CostBasedRateLimiter:
    ...
    async def check_quota(
        ...
        if usage_key not in self.quotas:
            self.quotas[usage_key] = {"used": 0, "reset_at": self._get_next_reset_time()}
