...
async def test_classification():
    ...
    for query in test_queries:
        ...
        print(
            f"Distribution: {', '.join([f'{p.value}: {s:.1%}' for p, s in result.distribution.items()])}"
        )
        print(
            f"Features: urgency={result.features.urgency_score:.2f}, complexity={result.features.complexity_score:.2f}, emotion={result.features.emotional_valence:.2f}"
        )
        if result.reasoning.get(result.primary_paradigm):
            print(
                f"Reasoning: {'; '.join(result.reasoning[result.primary_paradigm][:2])}"
            )
