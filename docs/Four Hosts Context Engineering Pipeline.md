...
from classification_engine import (
    HostParadigm, ClassificationResult, QueryFeatures
)
...
class ContextLayer(ABC):
    ...
    @abstractmethod
    async def process(self, classification: ClassificationResult, 
                     previous_outputs: Dict[str, Any] = None) -> Any:
...
class WriteLayer(ContextLayer):
    ...
    def __init__(self):
        ...
        self.paradigm_strategies = {
            HostParadigm.DOLORES: {
                ...
                'themes': ['oppression', 'resistance', 'truth', 'justice', 'revolution'],
                'narrative': 'victim-oppressor dynamics',
                'priorities': ['whistleblower accounts', 'leaked documents', 
                             'investigative reports', 'historical parallels']
            },
            HostParadigm.TEDDY: {
                'focus': 'Create empathetic profiles and document care needs',
                'themes': ['compassion', 'support', 'healing', 'community', 'protection'],
                'narrative': 'helper-recipient relationships',
                'priorities': ['personal stories', 'community resources', 
                             'best practices', 'success stories']
            },
            HostParadigm.BERNARD: {
                'focus': 'Systematic documentation of variables and evidence',
                'themes': ['analysis', 'causation', 'correlation', 'methodology', 'validation'],
                'narrative': 'hypothesis-evidence framework',
                'priorities': ['peer-reviewed studies', 'data sets', 
                             'meta-analyses', 'reproducible methods']
            },
            HostParadigm.MAEVE: {
                'focus': 'Map strategic landscape and actionable opportunities',
                'themes': ['leverage', 'advantage', 'optimization', 'influence', 'control'],
                'narrative': 'competitor-opportunity matrix',
                'priorities': ['case studies', 'market analysis', 
                             'implementation guides', 'success metrics']
            }
        }
    
    async def process(self, classification: ClassificationResult,
                     previous_outputs: Dict[str, Any] = None) -> WriteLayerOutput:
    ...
    def _extract_query_themes(self, classification: ClassificationResult) -> List[str]:
        ...
        important_tokens = [
            t for t in classification.features.tokens 
    ...
    def _generate_search_priorities(self, classification: ClassificationResult,
                                  base_priorities: List[str]) -> List[str]:
...
class SelectLayer(ContextLayer):
    ...
    def __init__(self):
        ...
        self.selection_strategies = {
            HostParadigm.DOLORES: {
                'query_modifiers': ['expose', 'reveal', 'uncover', 'truth about', 'scandal'],
                'source_types': ['investigative', 'alternative', 'whistleblower', 'activist'],
                'exclude': ['corporate PR', 'government propaganda', 'sponsored content'],
                'tools': ['deep_web_search', 'document_analysis', 'fact_checking'],
                'max_sources': 100
            },
            HostParadigm.TEDDY: {
                'query_modifiers': ['support', 'help', 'resources', 'guide', 'community'],
                'source_types': ['nonprofit', 'community', 'educational', 'testimonial'],
                'exclude': ['commercial', 'exploitative', 'sensational'],
                'tools': ['resource_finder', 'community_search', 'support_networks'],
                'max_sources': 75
            },
            HostParadigm.BERNARD: {
                'query_modifiers': ['research', 'study', 'analysis', 'data', 'evidence'],
                'source_types': ['academic', 'peer-reviewed', 'governmental', 'statistical'],
                'exclude': ['opinion', 'anecdotal', 'unverified'],
                'tools': ['academic_search', 'data_analysis', 'citation_network'],
                'max_sources': 150
            },
            HostParadigm.MAEVE: {
                'query_modifiers': ['strategy', 'tactics', 'competitive', 'optimize', 'leverage'],
    ...
    async def process(self, classification: ClassificationResult,
                     previous_outputs: Dict[str, Any] = None) -> SelectLayerOutput:
        ...
        if classification.secondary_paradigm and \
           classification.distribution[classification.secondary_paradigm] > 0.25:
    ...
    def _generate_search_queries(self, classification: ClassificationResult,
                               strategy: Dict[str, Any],
                               write_output: Optional[WriteLayerOutput]) -> List[Dict[str, Any]]:
        ...
        for modifier in strategy['query_modifiers'][:3]:
            ...
            queries.append({
                ...
                'weight': 0.8,
                'source_filter': strategy['source_types'][0] if strategy['source_types'] else None
            })
    ...
    def _generate_secondary_queries(self, classification: ClassificationResult,
                                  secondary_paradigm: HostParadigm) -> List[Dict[str, Any]]:
    ...
    def __init__(self):
        ...
        self.compression_strategies = {
            HostParadigm.DOLORES: {
                ...
                'priorities': ['evidence of wrongdoing', 'victim testimonies', 
                             'power dynamics', 'calls to action'],
                'remove': ['neutral descriptions', 'corporate justifications', 
                          'technical details', 'sidebar information']
            },
            HostParadigm.TEDDY: {
                'ratio': 0.6,  # Keep 60% - preserve human stories
                'strategy': 'narrative_preservation',
                'priorities': ['personal stories', 'emotional context', 
                             'support resources', 'positive outcomes'],
                'remove': ['statistics without context', 'technical jargon', 
                          'impersonal data', 'negative speculation']
            },
            HostParadigm.BERNARD: {
                'ratio': 0.5,  # Keep 50% - maximum pattern extraction
                'strategy': 'data_distillation',
                'priorities': ['statistical findings', 'causal relationships', 
                             'methodology', 'reproducible results'],
                'remove': ['anecdotes', 'opinions', 'emotional language', 
                          'unverified claims']
            },
            HostParadigm.MAEVE: {
                'ratio': 0.4,  # Keep 40% - only actionable intelligence
                'strategy': 'action_extraction',
                'priorities': ['specific tactics', 'success metrics', 
                             'implementation steps', 'competitive advantages'],
                'remove': ['background theory', 'historical context', 
                          'philosophical discussion', 'general advice']
            }
        }
    
    async def process(self, classification: ClassificationResult,
                     previous_outputs: Dict[str, Any] = None) -> CompressLayerOutput:
        ...
        logger.info(f"Compress layer set ratio: {output.compression_ratio:.0%}, "
...
class IsolateLayer(ContextLayer):
    ...
    def __init__(self):
        ...
        self.isolation_strategies = {
            HostParadigm.DOLORES: {
                ...
                'criteria': ['systemic patterns', 'power imbalances', 
                           'victim impact', 'accountability gaps'],
                'patterns': [r'pattern of\s+\w+', r'systematic\s+\w+', 
                           r'repeated\s+\w+', r'history of\s+\w+'],
                'focus': ['root causes', 'responsible parties', 'impact scale', 
            ...
            HostParadigm.TEDDY: {
                ...
                'criteria': ['individual needs', 'available support', 
                           'success stories', 'care strategies'],
                'patterns': [r'help(?:ing|s)?\s+\w+', r'support(?:ing|s)?\s+\w+', 
                           r'care\s+for\s+\w+', r'protect(?:ing|s)?\s+\w+'],
                'focus': ['immediate needs', 'long-term support', 'community resources', 
                         'individual dignity'],
                'structure': {
                    'needs_identified': [],
                    'resources_available': [],
            ...
            HostParadigm.BERNARD: {
                ...
                'criteria': ['statistical significance', 'causal links', 
                           'research gaps', 'methodological strengths'],
                'patterns': [r'\d+%\s+of\s+\w+', r'correlation\s+between', 
                           r'study\s+found', r'evidence\s+suggests'],
                'focus': ['key findings', 'statistical patterns', 'knowledge gaps', 
                         'future research'],
                'structure': {
                    'empirical_findings': [],
            ...
            HostParadigm.MAEVE: {
                ...
                'criteria': ['competitive advantages', 'implementation tactics', 
                           'success metrics', 'resource requirements'],
                'patterns': [r'strategy\s+to\s+\w+', r'tactic\s+for\s+\w+', 
                           r'advantage\s+of\s+\w+', r'optimize\s+\w+'],
                'focus': ['quick wins', 'long-term positioning', 'resource allocation', 
                         'success measurement'],
                'structure': {
    ...
    async def process(self, classification: ClassificationResult,
                     previous_outputs: Dict[str, Any] = None) -> IsolateLayerOutput:
...
class ContextEngineeringPipeline:
    ...
    async def process_query(self, classification: ClassificationResult) -> ContextEngineeredQuery:
        """Process classification through all context layers"""
        start_time = datetime.now()
        
        logger.info(f"Starting context engineering for paradigm: {classification.primary_paradigm.value}")
...
- Isolation Strategy: {eq.isolate_output.isolation_strategy}
    ...
    def export_engineered_queries(self, filepath: str):
        """Export engineered queries for analysis"""
        data = []
        ...
        for eq in self.processing_history:
            data.append({
                ...
                'paradigm': eq.classification.primary_paradigm.value,
                'confidence': eq.classification.confidence,
                'write_focus': eq.write_output.documentation_focus,
                'search_queries': len(eq.select_output.search_queries),
                'compression_ratio': eq.compress_output.compression_ratio,
                'token_budget': eq.compress_output.token_budget,
                'isolation_strategy': eq.isolate_output.isolation_strategy,
                'processing_time': eq.processing_time,
        ...
        logger.info(f"Exported {len(data)} engineered queries to {filepath}")

# --- Integration with Classification Engine ---

class IntegratedResearchPipeline:
    """Integrates Classification Engine with Context Engineering Pipeline"""
    
    def __init__(self, use_llm: bool = True):
        # Import classification engine
        from classification_engine import ClassificationEngine
        
        self.classification_engine = ClassificationEngine(use_llm=use_llm)
        self.context_pipeline = ContextEngineeringPipeline()
        
    async def process_research_query(self, query: str) -> ContextEngineeredQuery:
        """Complete pipeline from query to engineered output"""
        # Step 1: Classify the query
        classification = await self.classification_engine.classify_query(query)
        
        # Step 2: Process through context engineering
        engineered_query = await self.context_pipeline.process_query(classification)
        
        return engineered_query
    
    def get_full_metrics(self) -> Dict[str, Any]:
        """Get metrics from both engines"""
        return {
            'classification': self.classification_engine.get_classification_metrics(),
            'context_engineering': self.context_pipeline.get_pipeline_metrics()
        }
...
async def test_context_pipeline():
    ...
    engine = ClassificationEngine(use_llm=False)  # Use rule-based for testing
    ...
    for query in test_queries:
        ...
        engineered = await pipeline.process_query(classification)
        ...
        print(f"\nTotal Processing Time: {engineered.processing_time:.2f}s")
        print("-" * 80)
