...
class WebhookManager:
    ...
    async def trigger_event(
        ...
        logger.info(
            f"Triggered event {event} for {len(triggered_webhooks)} webhooks"
        )
    ...
    async def _deliver_webhook(self, delivery_id: str):
        ...
        try:
            ...
            logger.error(
                f"Failed to deliver webhook {delivery.id}: {e}"
            )
            ...
            if delivery.attempts < webhook.retry_policy["max_attempts"]:
                delay = min(
                    ...
                    (webhook.retry_policy["exponential_base"] ** (delivery.attempts - 1)),
...
def create_webhook_router(webhook_manager: WebhookManager) -> APIRouter:
    ...
    async def get_webhook(
        ...
        return {
            "webhook": webhook,
            "recent_deliveries": webhook_manager.get_webhook_deliveries(webhook_id, limit=10)
        }
