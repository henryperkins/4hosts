...
class PDFExporter(BaseExporter):
    ...
    async def export(self, research_data: Dict[str, Any]) -> ExportResult:
        ...
        # Add title
        title = self.options.custom_title or f"Research Report: {research_data.get('query', 'Unknown Query')}"
        ...
        filename = f"research_report_{research_data.get('id', 'unknown')}_{timestamp}.pdf"
    ...
    def _build_metadata_section(self, research_data: Dict[str, Any]) -> List:
        ...
        metadata_data = [
            ...
            ["Confidence Score", f"{research_data.get('confidence_score', 0) * 100:.1f}%"]
        ]
    ...
    def _build_answer_section(self, answer: Dict[str, Any], paradigm: str) -> List:
        ...
        # Get paradigm style
        paradigm_style = self.styles.get(f'Paradigm_{paradigm}', self.styles['Heading2'])
        ...
        if "sections" in answer:
            for section in answer["sections"]:
                elements.append(Paragraph(section.get("title", ""), self.styles['Heading3']))
                elements.append(Paragraph(section.get("content", ""), self.styles['BodyText']))
                
                # Add bullet points if present
                if "points" in section:
                    bullet_items = []
                    for point in section["points"]:
                        bullet_items.append(ListItem(Paragraph(point, self.styles['Normal'])))
        ...
        if "key_insights" in answer:
            ...
            for insight in answer["key_insights"]:
                insights_items.append(ListItem(Paragraph(insight, self.styles['Normal'])))
    ...
    def _build_sources_section(self, sources: List[Dict[str, Any]]) -> List:
        ...
        for i, source in enumerate(sources[:20], 1):  # Limit to 20 sources
            ...
            if source.get('summary'):
                elements.append(Paragraph(source['summary'][:200] + "...", self.styles['Citation']))
    ...
    def _add_header_footer(self, canvas, doc):
        ...
        canvas.drawString(inch, doc.height + doc.topMargin - 0.5*inch, "Four Hosts Research Report")
        canvas.drawRightString(doc.width + doc.leftMargin, doc.height + doc.topMargin - 0.5*inch, 
        ...
        if self.options.custom_footer:
            ...
            canvas.drawString(inch, 0.5*inch, "Generated by Four Hosts Research System")
...
class JSONExporter(BaseExporter):
    ...
    async def export(self, research_data: Dict[str, Any]) -> ExportResult:
        ...
        # Create filename
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        filename = f"research_data_{research_data.get('id', 'unknown')}_{timestamp}.json"
...
class CSVExporter(BaseExporter):
    ...
    async def export(self, research_data: Dict[str, Any]) -> ExportResult:
        ...
        if "sources" in research_data and self.options.include_sources:
            ...
            fieldnames = ["index", "title", "url", "relevance_score", "credibility_score", "summary"]
            ...
            for i, source in enumerate(research_data["sources"], 1):
                writer.writerow({
                    ...
                    "summary": source.get("summary", "")[:500]  # Truncate long summaries
                })
            ...
            writer.writerow(["Confidence Score", research_data.get("confidence_score", 0)])
        ...
        # Create filename
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        filename = f"research_export_{research_data.get('id', 'unknown')}_{timestamp}.csv"
...
class ExcelExporter(BaseExporter):
    ...
    async def export(self, research_data: Dict[str, Any]) -> ExportResult:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            # Overview sheet
            overview_data = {
                "Field": ["Research ID", "Query", "Paradigm", "Depth", "Confidence Score", "Sources Analyzed", "Generated At"],
                "Value": [
        ...
        # Create filename
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        filename = f"research_analysis_{research_data.get('id', 'unknown')}_{timestamp}.xlsx"
...
class MarkdownExporter(BaseExporter):
    ...
    async def export(self, research_data: Dict[str, Any]) -> ExportResult:
        ...
        if self.options.include_metadata:
            ...
            lines.append(f"- **Paradigm**: {research_data.get('paradigm', 'N/A').title()}")
            lines.append(f"- **Depth**: {research_data.get('depth', 'N/A')}")
            lines.append(f"- **Confidence Score**: {research_data.get('confidence_score', 0) * 100:.1f}%")
            lines.append(f"- **Sources Analyzed**: {research_data.get('sources_count', 0)}")
        ...
        if self.options.include_citations and "citations" in research_data:
            ...
            for citation in research_data["citations"]:
                lines.append(f"> {citation.get('text', '')} [{citation.get('source_index', '?')}]")
                lines.append("")
        ...
        # Create filename
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        filename = f"research_report_{research_data.get('id', 'unknown')}_{timestamp}.md"
