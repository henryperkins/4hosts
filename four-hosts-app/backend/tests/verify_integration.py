...
class IntegrationVerifier:
        ...
        try:
            ...
            helpers = [
                (
                    ...
                    lambda: main.generate_paradigm_queries("test", "bernard"),
                ),
                (
                    "get_paradigm_approach",
                    lambda: main.get_paradigm_approach(main.Paradigm.DOLORES),
                ),
                (
                    "get_paradigm_focus",
                    lambda: main.get_paradigm_focus(main.Paradigm.TEDDY),
                ),
                (
                    "generate_paradigm_summary",
                    lambda: main.generate_paradigm_summary("test", main.Paradigm.MAEVE),
                ),
            ]
    ...
    def verify_endpoints(self):
        ...
        try:
            ...
            if main.RESEARCH_FEATURES:
                research_endpoints = [("/sources/credibility/{domain}", ["GET"])]
                print("\n  Research Endpoints:")
    ...
    async def verify_integration_flow(self):
        ...
        try:
            ...
            queries = main.generate_paradigm_queries(
                query, classification.primary.value
            )
            print_test(
    ...
    def generate_report(self):
        ...
        if percentage == 100:
            ...
            print(
                f"{YELLOW}⚠ MOSTLY INTEGRATED{RESET} ({passed_tests}/{total_tests} - {percentage:.1f}%)"
            )
            print(f"\nThe core functionality works but some features may be limited.")
        else:
            print(
                f"{RED}✗ INTEGRATION ISSUES{RESET} ({passed_tests}/{total_tests} - {percentage:.1f}%)"
            )
            print(
