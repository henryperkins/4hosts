...
class SourceReputationDatabase:
    ...
    async def calculate_credibility_score(
        self, domain: str, paradigm: str = "bernard"
    ) -> CredibilityScore:
        ...
        async with self.domain_checker:
            ...
            bias_rating, bias_score, fact_check_rating = (
                await self.bias_detector.get_bias_score(domain)
            )
...
async def get_source_credibility(
    domain: str, paradigm: str = "bernard"
) -> CredibilityScore:
...
async def test_credibility_system():
    ...
    for domain in test_domains:
        ...
        for paradigm in ["dolores", "teddy", "bernard", "maeve"]:
            ...
            print(
                f"  {paradigm.upper()}: {credibility.overall_score:.2f} "
