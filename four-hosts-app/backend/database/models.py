...
user_saved_searches = Table(
    ...
    Column('research_id', UUID(as_uuid=True), ForeignKey('research_queries.id', ondelete='CASCADE')),
    Column('saved_at', DateTime(timezone=True), default=lambda: datetime.now(timezone.utc)),
...
webhook_events = Table(
    ...
    Base.metadata,
    Column('webhook_id', UUID(as_uuid=True), ForeignKey('webhooks.id', ondelete='CASCADE')),
...
class User(Base):
    ...
    saved_searches = relationship("ResearchQuery", secondary=user_saved_searches, back_populates="saved_by_users")
...
class ResearchQuery(Base):
    ...
    saved_by_users = relationship("User", secondary=user_saved_searches, back_populates="saved_searches")
    sources = relationship("ResearchSource", back_populates="research_query", cascade="all, delete-orphan")
    answer = relationship("ResearchAnswer", back_populates="research_query", uselist=False, cascade="all, delete-orphan")
    exports = relationship("Export", back_populates="research_query", cascade="all, delete-orphan")
    webhook_deliveries = relationship("WebhookDelivery", back_populates="research_query", cascade="all, delete-orphan")
...
class ResearchSource(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    research_id = Column(UUID(as_uuid=True), ForeignKey('research_queries.id', ondelete='CASCADE'), nullable=False)
...
class ResearchAnswer(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    research_id = Column(UUID(as_uuid=True), ForeignKey('research_queries.id', ondelete='CASCADE'), nullable=False, unique=True)
...
class Citation(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    answer_id = Column(UUID(as_uuid=True), ForeignKey('research_answers.id', ondelete='CASCADE'), nullable=False)
...
class WebhookDelivery(Base):
    ...
    research_id = Column(UUID(as_uuid=True), ForeignKey('research_queries.id', ondelete='SET NULL'))
